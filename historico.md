# Histórico de Mudanças no Perfil

> Changelog de alterações em princípios (relevância, confiança) e o motivo.
> Uso pessoal, para auditoria. Não é base para recomendações — serve para eu
> entender, no futuro, por que o perfil mudou, e para evitar reabrir decisões
> já tomadas.
>
> Não registrar aqui adições de jogos comuns — apenas mudanças de princípio.

## Template

## AAAA-MM-DD
### Nome do princípio
Relevância: X → Y (ou Confiança: A → B)
**Motivo:**

---

## 2026-07-06
### Setup inicial
Perfil e princípios criados a partir de conversas anteriores e da base de
jogos avaliados. Sem alterações de princípio registradas ainda.

## 2026-07-06
### Refinamentos a partir de conversa (ONI, Nioh, MMOs)
- **Mundo interessante:** acrescentada a noção de mundo conectado/coeso como
  fator (contraexemplo: Nioh, missões soltas).
- **Mecânicas profundas:** documentado o teto de complexidade (ONI).
- **MMOs:** registrado como interesse recorrente (exceção à jornada completa),
  com p2w como dealbreaker e WoW como exceção.
- **Novo antipadrão:** "Sandbox aberto / falta de objetivos claros"
  (ONI, Minecraft, Tropico free-play).
Nenhuma relevância ou confiança foi alterada nesta rodada.

## 2026-07-31
### Novo princípio: Tensão na travessia
Relevância: — → 3 | Confiança: — → Média
**Motivo:**
Separado de "Atmosfera", que estava misturando duas coisas. Atmosfera é
estética; isto é mecânico (mobilidade baixa, recurso drenando, custo real em
avançar). A distinção apareceu ao explicar por que Sekiro, apesar de nota 9,
fica abaixo dos outros Souls: tem combate, história e exploração boas, mas
mobilidade alta e retorno barato eliminam a tensão do deslocamento.
Não é caso isolado — Souls, System Shock, Prey, STALKER, Metro, Sunless Sea e
DREDGE sustentam. Relevância 3 (e não 4) porque Zelda, meu jogo preferido, não
tem nada disso, e Sekiro tirou 9 sem: é amplificador, mesma natureza de
Atmosfera, que também é 3. Confiança Média porque a fronteira com Atmosfera
acabou de ser traçada e ainda não foi testada em outros casos.

### Novo antipadrão: Atrito fabricado / mecânicas de retenção
**Motivo:**
Formalização do que eu chamava de "vibe de jogo mobile". O corte não é estético
nem "o jogo cobra": é o jogo fabricar um problema para vender a solução
(energia/stamina, taxa de sucesso arbitrária com atalho pago) ou converter
jogar em obrigação de calendário (login diário). Distinto de P2W: P2W é sobre
vantagem, este é sobre desenho. Escopo limitado a MMO/F2P por ora — só existe
um caso fora dele (Bloons TD 6), e um caso não faz padrão.

## 2026-09-24
### Mistérios com conclusão
Relevância: 2 → 3 | Confiança: Média (sem alteração)
**Motivo:**
Clair Obscur: Expedition 33 virou meu jogo preferido, tomando o lugar de
Zelda, e o que o colocou acima foi justamente a história — Zelda ganha em
exploração e puzzles, mas tem história mediana. Somado a Sunless Sea e Disco
Elysium, o princípio passa a ter três casos, dois deles nota 10.
Subiu para 3 e não mais porque continua sendo amplificador, não requisito:
Zelda foi meu favorito por anos com história mediana e Dark Souls funciona com
narrativa fragmentada. Fica no mesmo nível dos outros amplificadores
(Atmosfera e Tensão na travessia, ambos 3).
Descrição e critério mantidos sem alteração.
